...
export default function FilterAccordion() {
  ...
  function handleSelect(filterKey: string, value: string) {
    ...
    if (filterKey === 'status') {
      // Status always requires a value — only set if value is non-empty
      if (value) params.set(filterKey, value)
    } else {
      // Empty string means the item was deselected (same item clicked again)
      if (!value) {
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      >
        {sections
          .filter((i) => !(i.id === 'design' && category !== 'corporates' && category !== 'casuals'))
          .map((section) => (
            <AccordionItem
              key={section.id}
            ...
            >
              <FilterList
                selectedKey={section.id === 'status' ? status : section.id === 'price' ? price : section.id === 'design' ? design : ''}
